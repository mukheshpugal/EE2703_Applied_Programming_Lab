...
# A class to hold circuit information
class Circuit:
	...
	def solve(self):
		...
		self.nodeV = [self.nodes[i] + " : " + str(np.around(self.solution[i], 10)) for i in range(n)]
		self.iThroughV = [self.vSources[i].name + " : " + str(np.around(self.solution[i + n], 10)) for i in range(k)]
...
# A class to hold contents of a component
class Component:
	...
	def __init__(self, listIn):
		...
		# Nodes have to be strings as per this program
...
if __name__ == "__main__":
	...
	# Copying netlist file to a list
	try:
	...
	circuit = Circuit(lines, '1') # The constructor handles parsing
